...
def create_mesh(
    para, image, map, N=512, max_batch=32 ** 3, offset=None, scale=None, output_mesh = False, filename = None
):
    start = time.time()
    ...
    if sdf_values.max() <= 0 or sdf_values.min() >= 0:
        return 0, 0, 0, 0, 0
    ...
    if output_mesh is False:
        ...
        print(end-start)
        return verts, faces, samples, next_indices, 1
